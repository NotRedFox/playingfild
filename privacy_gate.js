/**
 * Sensitive-page guard + privacy gate for content scripts.
 * Defines __pfComputeSensitivePageActive and __pfSensitivePageActive (single manifest file).
 */
(function () {
  if (globalThis.__pfSensitivePageGuardLoaded) return;
  globalThis.__pfSensitivePageGuardLoaded = true;

  // Keep in sync with excluded_hosts.js categories.
...
(function () {
  if (globalThis.__pfPrivacyGateLoaded) return;
  globalThis.__pfPrivacyGateLoaded = true;

  const compute = globalThis.__pfComputeSensitivePageActive;
  if (typeof compute === 'function') {
    globalThis.__pfSensitivePageActive = compute();
  } else {
    globalThis.__pfSensitivePageActive = true;
    console.warn('[pf-privacy] sensitive page guard unavailable — fail-closed');
  }
})();
